...
/** Call after link-accounts (or skip) so `/dashboard` and FinPilot shell unlock. */
export function completeFinPilotOnboardingUnlock() {
  try {
...
export function getDashboardBlockRedirect() {
  if (typeof localStorage === "undefined") return "/";
  if (localStorage.getItem(FINPILOT_ONBOARDING_KEY) === "true") return null;
  if (localStorage.getItem(PENDING_LINK_ACCOUNTS_KEY) === "true") {
    return "/link-accounts";
  }
  if (localStorage.getItem(INAPP_READY_FOR_LINK_KEY) === "true") {
